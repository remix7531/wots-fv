/* Minimal, self-contained SHA-256 (FIPS 180-4).
   Will be treated as admitted in the VST proof (its specification is
...
void sha256(uint8_t out[SHA256_DIGEST_SIZE], const uint8_t *in, size_t inlen) {
    ...
    size_t full_blocks = inlen / 64;
    for (size_t i = 0; i < full_blocks; i++) {
        sha256_compress(H, in + 64 * i);
    }

    uint8_t tail[128];
    size_t rem = inlen - full_blocks * 64;
    memcpy(tail, in + full_blocks * 64, rem);
    tail[rem] = 0x80;
    size_t pad_end = (rem < 56) ? 64 : 128;
    for (size_t i = rem + 1; i < pad_end - 8; i++) tail[i] = 0;
    for (int i = 0; i < 8; i++) {
        tail[pad_end - 1 - i] = (uint8_t)(bitlen >> (8 * i));
    }
